...
class TrafficMonitorRest13(simple_switch_13.SimpleSwitch13):
    ...
    def __init__(self, *args, **kwargs):
        ...
        wsgi = kwargs['wsgi']
        wsgi.register(TrafficMonitorController, {
                      traffic_monitor_instance_name: self})
    ...
    def _flow_stats_reply_handler(self, ev):
        body = ev.msg.body
        self.flowStat = json.dumps(ev.msg.to_jsondict(
        ), ensure_ascii=True, indent=3, sort_keys=True)

    @set_ev_cls(ofp_event.EventOFPPortStatsReply, MAIN_DISPATCHER)
    def _port_stats_reply_handler(self, ev):
        body = ev.msg.body
        self.portStat = json.dumps(ev.msg.to_jsondict(
        ), ensure_ascii=True, indent=3, sort_keys=True)
...
class TrafficMonitorController(ControllerBase):
    ...
    def __init__(self, req, link, data, **config):
        super(TrafficMonitorController, self).__init__(
            req, link, data, **config)
        self.simple_switch_app = data[traffic_monitor_instance_name]
        print(self.simple_switch_app)

    @route('trafficmonitor', portStatUrl, methods=['GET'])
    def list_Port_Stat(self, req, **kwargs):
        print(self.simple_switch_app.portStat)
        return Response(charset='UTF-8', content_type='application/json', text=self.simple_switch_app.portStat)

    @route('trafficmonitor', flowStatUrl, methods=['GET'])
    def list_Flow_Stat(self, req, **kwargs):
        return Response(charset='UTF-8', content_type='application/json', text=self.simple_switch_app.flowStat)
