...
class shortest_path(app_manager.RyuApp):
    ...
    def switch_features_handler(self, ev):
        ...
        action = ofp_parser.OFPInstructionActions(
            ofp.OFPIT_APPLY_ACTIONS, [ofp_parser.OFPActionOutput(ofp.OFPP_CONTROLLER)])
        inst = [action]
    ...
    def packet_in_handler(self, ev):
        ...
        if self.net.has_node(pkt_ethernet.dst):
            ...
            path = nx.shortest_path(
                self.net, pkt_ethernet.src, pkt_ethernet.dst)
            ...
            for on_path_switch in range(1, len(path)-1):
                ...
                action = ofp_parser.OFPInstructionActions(
                    ofp.OFPIT_APPLY_ACTIONS, [ofp_parser.OFPActionOutput(next_port)])
                inst = [action]
                self.add_flow(
                    dp=self.switch_map[now_switch], match=next_match, inst=inst, table=0)

                action = ofp_parser.OFPInstructionActions(
                    ofp.OFPIT_APPLY_ACTIONS, [ofp_parser.OFPActionOutput(back_port)])
                inst = [action]
                self.add_flow(
                    dp=self.switch_map[now_switch], match=back_match, inst=inst, table=0)
